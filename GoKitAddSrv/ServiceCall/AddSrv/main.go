...
import (
	...
	trimMinddle "gokitaddsrv/minddleware/trim"
...
func main() {
	...
	// trim service client
	conn, err := grpc.Dial("localhost:8975", grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
		fmt.Fprintf(os.Stderr, "unable to connect to trim service: %v\n", err)
		os.Exit(1)
	}
	defer conn.Close()

	trimEndpoint := trimMinddle.MakeTrimEndpoint(conn)
	svc = trimMinddle.NewServiceWithTrim(trimEndpoint, svc)
