...
func main() {
	...
	// trim service client
	// 使用Consul进行服务发现
	trimEndpoint, err := trimMinddle.GetTrimServiceFromConsul("localhost:8500", "trim_service", []string{"trim"}, logger)
	if err != nil {
		fmt.Fprintf(os.Stderr, "unable to get trim service from consul: %v\n", err)
		os.Exit(1)
	}
